...
test("claudeCode: true when symlink exists at agentsSkillsDir/{ns}", async () => {
  ...
  const state = await getDistributionState(repoPath, { agentsSkillsDir: agentsDir, opencodeJsonPath: opencodeJson, openclawSkillsDir: openclawDir });
...
test("applyDistributionChanges: enable claudeCode creates symlink", async () => {
  ...
  const result = await applyDistributionChanges(
    repoPath,
...
test("applyDistributionChanges: enable openCode adds plugin to opencode.json", async () => {
  ...
  await applyDistributionChanges(
    repoPath,
