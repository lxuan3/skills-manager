...
export async function getDistributionState(repoPath, opts = {}) {
  const { agentsSkillsDir, opencodeJsonPath, openclawSkillsDir, geminiSkillsDir, codexSkillsDir } = await resolveOpts(opts);
  ...
  const namespaces = submodules.map((sub) => {
    ...
    return {
      ...
      tools: {
        ...
        codex: codexSkillsDir ? symlinkExists(path.join(codexSkillsDir, ns)) : "na",
      },
    };
  });
  ...
  for (const { namespace, tool, enabled } of sorted) {
    ...
    try {
      if (tool === "claudeCode") {
        ...
        if (!codexSkillsDir) {
          errors.push({ namespace, tool, error: "Codex skills directory not configured" });
          continue;
        }
        const linkPath = path.join(codexSkillsDir, namespace);
        if (enabled) {
