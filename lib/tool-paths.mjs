...
function detectGemini({ platform, home, userprofile }) {
  const candidate =
    platform === "win32"
      ? path.join(userprofile, ".gemini/skills")
...
function detectCodex({ platform, home, userprofile }) {
  const candidate =
    platform === "win32"
 ...
 * @returns {{ claudeCode: string|null, openCode: string|null, openClaw: string|null, gemini: string|null }}
 */
 ...
 */
const KNOWN_TOOL_KEYS = ["claudeCode", "openCode", "openClaw", "gemini", "codex"];
...
export async function getToolPaths(opts = {}) {
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (raw?.tools) {
      // If all known keys are present, return as-is; otherwise re-detect missing ones
      const missing = KNOWN_TOOL_KEYS.filter((k) => !(k in raw.tools));
      if (missing.length === 0) return raw.tools;
      const detected = detectToolPaths(opts);
      const merged = { ...raw.tools };
      for (const k of missing) merged[k] = { path: detected[k], manual: false };
      await saveToolPaths(merged, { configPath });
      return merged;
    }
  ...
  const tools = {
    ...
    gemini: { path: detected.gemini, manual: false },
    codex: { path: detected.codex, manual: false },
  };
