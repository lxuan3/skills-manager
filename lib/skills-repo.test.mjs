...
test("listSubmodules returns submodule names and urls", () => {
  ...
  try {
    ...
    assert.ok(result[0].skillsPath.endsWith("third-party/superpowers/skills"), "skillsPath should point to skills/ subdir");
  } finally {
...
test("listSubmodules returns empty array when .gitmodules is absent", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skills-no-gitmodules-"));
  try {
    const result = listSubmodules(dir);
