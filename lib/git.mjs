...
// Wrapper that always allows file:// transport so tests (which use bare
// local repos as submodule upstreams) work even when the global git config
// has `protocol.file.allow = user` (the macOS default since git 2.38).
...
export function fetchSubmodule(repoPath, submoduleName) {
  ...
  // Get local HEAD
  const localSha = git(["rev-parse", "HEAD"], { cwd: subPath }).trim();
