...
/**
 * Remove a submodule completely from the repository.
 * Runs: git submodule deinit -f <path>, git rm -f <path>,
 * then deletes .git/modules/<path>.
 * @param {string} repoPath - absolute path to the git repo root
 * @param {string} submodulePath - path value from .gitmodules (e.g. "third-party/superpowers")
 */
export function removeSubmodule(repoPath, submodulePath) {
  git(["submodule", "deinit", "-f", submodulePath], { cwd: repoPath });
  git(["rm", "-f", submodulePath], { cwd: repoPath });
  const modulesPath = path.join(repoPath, ".git", "modules", submodulePath);
